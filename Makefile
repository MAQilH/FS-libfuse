...
$(MAIN_TARGET): $(MAIN_SRC)
	$(CC) $(CFLAGS) -lpthread -o $(MAIN_TARGET) $(MAIN_SRC)

# Build with debug symbols for profiling
profile: $(MAIN_SRC)
	$(CC) $(PROFILE_CFLAGS) -o $(MAIN_TARGET)_profile $(MAIN_SRC)

# Build benchmark for profiling (non-interactive stress test)
benchmark: $(MAIN_SRC) $(BENCH_SRC)
	$(CC) $(PROFILE_CFLAGS) -c $(MAIN_SRC) -o main_bench.o -DEXCLUDE_MAIN
	$(CC) $(PROFILE_CFLAGS) -o $(BENCH_TARGET) main_bench.o $(BENCH_SRC)
	rm -f main_bench.o

# Build with simple text-based profiler (shows bottleneck analysis)
benchmark_profile: $(MAIN_SRC) $(BENCH_SRC) simple_profiler.h
	$(CC) $(PROFILE_CFLAGS) -DENABLE_PROFILING -c $(MAIN_SRC) -o main_prof.o -DEXCLUDE_MAIN
	$(CC) $(PROFILE_CFLAGS) -DENABLE_PROFILING -o $(BENCH_TARGET)_profile main_prof.o $(BENCH_SRC)
	rm -f main_prof.o
	@echo ""
...
tracy: $(MAIN_SRC) $(BENCH_SRC) tracy_client.o
	...
	$(CXX) -o $(BENCH_TARGET)_tracy main_tracy.o bench_tracy.o tracy_client.o $(TRACY_LDFLAGS)
	rm -f main_tracy.o bench_tracy.o
...
$(TEST_TARGET): $(MAIN_SRC) $(TEST_SRC)
	...
	$(CC) $(CFLAGS) -lpthread -o $(TEST_TARGET) main_test.o $(TEST_SRC)
	rm -f main_test.o

# Build concurrency test (demonstrates race conditions without locks)
$(CONCURRENCY_TEST_TARGET): $(MAIN_SRC) $(CONCURRENCY_TEST_SRC)
	$(CC) $(CFLAGS) -c $(MAIN_SRC) -o main_concurrency.o -DEXCLUDE_MAIN
	$(CC) $(CFLAGS) -o $(CONCURRENCY_TEST_TARGET) main_concurrency.o $(CONCURRENCY_TEST_SRC)
	@rm -f main_concurrency.o

# Build lock weakness demo (demonstrates lock problems)
$(LOCK_WEAKNESS_TARGET): $(MAIN_SRC) $(LOCK_WEAKNESS_SRC)
	$(CC) $(CFLAGS) -lpthread -c $(MAIN_SRC) -o main_lock_demo.o -DEXCLUDE_MAIN
	$(CC) $(CFLAGS) -lpthread -o $(LOCK_WEAKNESS_TARGET) main_lock_demo.o $(LOCK_WEAKNESS_SRC)
